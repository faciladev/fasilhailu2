...
const Admin = () => {
  ...
  const onSubmit = async (data) => {
    const { title, url, technology, description, image } = data;
    const imageUrl = `https://${process.env.NEXT_PUBLIC_AWS_S3_BUCKET_NAME}.s3.amazonaws.com/${image[0].name}`;
    const variables = { title, url, technology, description, imageUrl };
    try {
      toast.promise(createProject({ variables }), {
  ...
  return (
    <div className="container mx-auto max-w-md py-12">
      <Toaster />
      <h1 className="text-3xl font-medium my-5">Create a new project</h1>
      <form
